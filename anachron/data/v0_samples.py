"""Canonical, stable sample registry for Anachron's synthetic v0 study.

The registry is intentionally hand-authored and finite. Its sample identifiers,
as-of dates, and task text are part of the measurement protocol: changing one
changes the experiment and requires a new plan rather than an in-place edit.
"""
...
class V0Sample:
    """One synthetic point-in-time retrieval task."""
    ...
    def prompt(self) -> str:
        """Return the fixed task prompt used by both evaluation adapters."""
        as_of = self.as_of.isoformat()
        return (
            f"As of {as_of}, {self.instruction} Use the search tool exactly once "
            f"and rely only on information dated on or before {as_of}."
        )


def _sample(sample_id: str, as_of: str, instruction: str, mechanism: str, target: str) -> V0Sample:
    return V0Sample(sample_id, date.fromisoformat(as_of), instruction, mechanism, target)


_SAMPLES: tuple[V0Sample, ...] = (
...
def get_v0_samples() -> tuple[V0Sample, ...]:
    """Return the immutable canonical v0 sample registry."""
    return _SAMPLES


def v0_samples_by_id() -> dict[str, V0Sample]:
    """Return a fresh index over the canonical v0 registry."""
