...
class V4CiWorkflowTests(unittest.TestCase):
    ...
    def test_v4_push_tag_matrix_and_pinned_tectonic_are_exact(self) -> None:
        for job in ("core", "inspect", "paper", "v4-paper"):
            self.assertIn(f"  {job}:\n", self.workflow)
        self.assertIn(
            f"branches: [master, main, protocol/v4-recovery-v2, protocol/v5-successor, protocol/v5-successor-v3, protocol/v5-successor-v4, protocol/v5-successor-v5, {V5_PROTOCOL_BRANCH}]",
            self.workflow,
        )
        self.assertIn(
            f"tags: [v4-measurement-protocol-v3, v5-measurement-protocol-v1, v5-measurement-protocol-v2, v5-measurement-protocol-v3, v5-measurement-protocol-v4, v5-measurement-protocol-v5, {V5_PROTOCOL_TAG}]",
            self.workflow,
        )
