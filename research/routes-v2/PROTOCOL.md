...
Raw Wikipedia discovery material from v1 may be used only as an input to fresh v2 revalidation. Revalidation retains full-revision hashes and immutable identities, never source text. A separate `routes-v2-excerpt-receipt-v1` derives one unique-anchor, Unicode-safe UTF-8 window per arm, capped at 4096 bytes; it records anchor/excerpt offsets and hashes plus the full-revision binding. Mapping inputs contain only question, anchors, bounded aliases, opaque IDs, and raw/revalidation bindings. Legacy full-content mappings are rejected. Draft creation, sealing, and replay admission must rederive every excerpt byte-for-byte from the exact ignored raw artifacts; raw files are never copied to the manifest, audit, or paper. Human decisions use the contract-frozen `routes-v2-source-decisions-v2` schema and bind the complete reviewed projection, validator ID, canonical UTC timestamp, and explicit certification. Only all-PASS yields the runnable manifest for that phase. Any REJECT yields a durable phase-local FAIL receipt and no manifest.
